  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-zinc-400 text-sm mt-1">Withdraw your SPARK tokens to external wallet</p>
      </div>

      <div className="bg-gradient-to-br from-purple-600/20 to-pink-600/20 border border-purple-600/20 rounded-xl p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-zinc-400 text-sm">Available SPARK Balance</p>
            <p className="text-3xl font-bold text-purple-400 mt-1">{sparkBalance.toLocaleString()} SPARK</p>
            <p className="text-xs text-zinc-500 mt-1">Min: 10 SPARK | Fee: 0%</p>
          </div>
          <div className="w-16 h-16 bg-purple-600/10 rounded-xl flex items-center justify-center">
            <Icon name="coins" size={32} className="text-purple-400" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {amount && Number(amount) >= 10 && (
              <div className="bg-zinc-800 rounded-lg p-3 text-sm space-y-1">
                <div className="flex justify-between text-zinc-400">
                  <span>Amount</span>
                  <span>{Number(amount).toLocaleString()} SPARK</span>
                </div>
                <div className="flex justify-between text-zinc-400">
                  <span>Fee</span>
                  <span>0 SPARK</span>
                ...
                <div className="border-t border-zinc-700 pt-1 flex justify-between text-white font-medium">
                  ...
                  <span className="text-purple-400">{Number(amount).toLocaleString()} SPARK</span>
                </div>
              </div>
            )}

            {walletAddress && amount && Number(amount) >= 10 && !otpVerified && (
        ...
        <Card title="SPARK Withdrawal History">
              ...
              {withdrawals.map((wd) => (
                <div key={wd.id} className="p-3 bg-zinc-800/50 rounded-lg">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium text-purple-400">{Number(wd.amount).toLocaleString()} SPARK</span>
                  </div>
                  ...
                  {wd.tx_hash && (
                    <a
                      href={`https://polygonscan.com/tx/${wd.tx_hash}`}
                      target="_blank"
