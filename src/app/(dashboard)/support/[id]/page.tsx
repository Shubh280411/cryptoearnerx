...
export default function TicketDetailPage() {
  ...
  if (!ticket) {
    return <div className="text-center py-12 text-zinc-400">Ticket not found</div>;
  }

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <a href="/support" className="text-zinc-400 hover:text-white">
          <Icon name="arrowLeft" size={20} />
        </a>
        <div>
          <h1 className="text-xl font-bold text-white">{ticket.subject}</h1>
          <p className="text-xs text-zinc-500 capitalize">{ticket.category} - {ticket.status}</p>
        </div>
      </div>
